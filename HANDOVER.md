...
- **DONE — tabbed place profile (Analisa.pt drawer pattern).** `analisa_pt_information_architecture.md` landed (English) and the drawer pattern was adapted to `/places/[slug]` — see "Tabbed place profile + metric benchmark bands" below. Changes are **staged in the working tree, not committed/pushed** — review then commit.
...
### Tabbed place profile + metric benchmark bands (Analisa.pt drawer) — NEW ✅
Adapted the Analisa.pt municipality-drawer IA to `/places/[slug]`. The profile is now a tabbed drawer with a persistent score-hero header; the old Simple⇄Comprehensive toggle is retired (tabs provide the depth, no info lost).
- **Tab structure** (`components/PlaceProfileClient.tsx`, horizontally scrollable `role=tablist`): **Overview** (composite breakdown bars + key facts + context entry-point buttons) → **persona lenses** (Family / Young professional / Retiree / Student, each re-weighting the seven scored domains via `personaWeights` with a factor breakdown + per-lens data coverage) → **scored-domain tabs** (Affordability, Transport, Crime/Safety, Health, Education, Hazards, Income) → **context tabs** (Home buyer, Walk &amp; cycle, Equity &amp; community, Data coverage). Existing components are folded into tabs (`HomeBuyerCard`, `WalkAccessPanel`, `CyclabilityPanel`, `ContextPanels`, `DataCoverageCard`, `DataConfidenceCard`, `ScoreBadge`/`DomainBar`, `SourceDrawer`, `ProfileEngagement`) — none duplicated. Composite/persona scores are labelled **optional lenses, not an authority**; context stays **not in score**.
- **Rich metric card** (`components/MetricCard.tsx`): name + value + unit, honest direction (higher/lower is better), GM **percentile**, a **benchmark band** (GM median + P25–P75 IQR over the raw indicator, with this area's value dot coloured by the YlGnBu percentile ramp), source link + `StalenessBadge` (period/stale). **No trend** — every card states "Single period — no trend data held" (we hold no time-series; trends are never fabricated). Missing indicators render an explicit "no data held" state.
- **GM benchmarks computed server-side** in `app/places/[slug]/page.tsx` via `computeGmBenchmarks(data.places)` (memoised at module scope, computed once for the whole export; dataset read memoised too). **No new fetched data file.** Metric metadata (label/unit/format/direction) is the single-source-of-truth catalog `lib/metric-catalog.ts`; stats helpers in `lib/benchmarks.ts` (`quantileSorted`, `computeBenchmark`, `computeGmBenchmarks`) unit-tested in `tests/benchmarks.test.ts`.
- **"Show on map" deep-link:** scored-domain tabs/cards link to `/?layer=<domain>`; parsed in `lib/share-url.ts` (`parseMapUrlState.layer`, validated against `V1_SCORED_DOMAINS`) and applied as a one-shot in `lib/use-map-personalisation.ts` (activates that choropleth). Plain weight/shortlist edits never carry it. Round-trip tested in `tests/share-url.test.ts`.
- **Accessibility:** proper `role=tablist/tab/tabpanel`, `aria-selected`, `aria-controls`/`aria-labelledby`, roving `tabIndex`, Arrow/Home/End keyboard nav, focus-visible rings via the `--focus` token, `prefers-reduced-motion` honoured (no smooth tab-scroll when reduced). Tab strip is horizontally scrollable (per the IA doc).
- **Map-home mini-profile unchanged:** the right-panel `ScoreBreakdownPanel` still links "Full profile →" to the tabbed page.
- **Methodology updated** with the tabbed-profile + benchmark-band + no-trend-data honesty note.
- **Gate green:** `typecheck`, `npm test` (43 tests), `eslint` (0 errors), `npm run build` → `out/` (363 pages). Static-export safe; base-path-aware. **Not committed/pushed — staged for review.**

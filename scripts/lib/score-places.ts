/**
 * Pure scoring assembly: indicators-raw places -> scored Place[] for a region.
 *
 * Extracted from scripts/score.ts so the region gating is unit-testable (the
 * script self-executes on import). Melbourne output is byte-identical to the
 ...
 * "vcsa-recorded-offences" / "vic-planning-bpa" / "vic-planning-flood" with
 * scored:true onto every region's all-null safety/hazards indicators.
 */
...
/** A domain whose source does not exist for this region: present so the UI can
 * say "not scored here", but honestly unscored - no percentile, no indicators,
 * and crucially no wrong-state sourceId. */
function unscoredDomain(domain: DomainId): DomainScore {
  return { domain, scored: false, percentile: null, subIndicators: {} };
}
...
/**
 * Score one region's raw indicator places. Pure: same inputs -> same output
 * (staleness compares source periods against the current UTC year).
 *
 * `periodById` is the sources.json id -> period map used for staleness flags;
 * pass an empty Map when the manifest is unavailable (all stale:false).
 */
export function scorePlaces(
  ...
  function isStale(sourceId: string): boolean {
    const period = periodById.get(sourceId);
  ...
  return raw.map((p) => {
    ...
    if (!nonResidential) {
      ...
      domains.safety = crimeAdapter
        ? domainScore("safety", safetyPct, {
            propertyCrime: indicator(
              p.propertyCrimeRate,
              propPct,
              crimeAdapter.sourceId,

...
// REGION_TYPE=SA2 . STATE=2 (Victoria). Decoding is by dimension id, so an
// over-broad key still decodes correctly; we filter VIC SA2s in the decoder.
const DATA_KEY = "3._T+1+2+3._T..SA2.2";
const G49_URL =
...
async function main() {
  ...
  const { places, qallpLabels } = decodeQualificationsSdmx(json);
  ...
  if (n === 0) {
    throw new Error("ABS C21_G49_SA2: decoded 0 Victorian SA2s - check the data key / response");
  }
  ...
  console.log(`Wrote ${dest}: ${n} Victorian SA2s`);
